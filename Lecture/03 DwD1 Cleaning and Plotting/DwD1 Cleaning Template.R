...
head(gm)
gm_long = pivot_longer(gm, contains('_'),
             names_to = c('.value', 'year'),
             names_sep = '_')
# tibbles!
# year is a character... we should fix that
...
#### 4.1) subsetting data ####
## ------------------------ ##
gm_2007 = filter(gm_long, year == 2007)
...
gm %>%
  ...
  mutate(gdp = gdpPercap*pop,
         year = as.numeric(year))
# note that we are only outputting into the console here.
# We did not save the output as an object
...
# I don't recommend doing all of this in one line
gm_long %>%
  mutate(year = as.numeric(year)) %>%
  group_by(continent, year) %>%
  summarise(avg_lifeExp = mean(lifeExp),
