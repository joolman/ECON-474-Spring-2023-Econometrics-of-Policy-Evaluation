...
marginaleffects(fit1_logit, newdata = 'mean')$dydx
...
# [TO BE]
# [ADDITIVE!!!!]
fit2_logit = glm(in_lf ~ age + yos + female + has_children + married + black + asian + hispanic +
                   moved_ly + density + ftotinc, data = acs, family = 'binomial')
marginaleffects(fit2_logit, newdata = 'mean')$dydx
fit2_ols = lm(in_lf ~ age + yos + female + has_children + married + black + asian + hispanic +
     moved_ly + density + ftotinc, data = acs)
fit2_ols
...
mean(yhat1 > 1 | yhat1 < 0 )

yhat2 = fit2_ols$fitted.values
mean(yhat2 > 1 | yhat2 < 0 )
