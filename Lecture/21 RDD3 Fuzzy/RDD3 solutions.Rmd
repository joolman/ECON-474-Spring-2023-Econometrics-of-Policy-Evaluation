...
```{r maimonides var}
df = df %>%
  mutate(iv_maimonides = maimonides_rule(enrollment))
```
...
```{r}
fit_ols = feols(avgmath ~ csw(classize, pct_disadv, enrollment), 
      data = df,
      cluster = ~schlcode)
```
...
```{r iv reduced form}
fit_iv_rf = feols(avgmath ~ csw(iv_maimonides, pct_disadv, enrollment), 
      data = df,
      cluster = ~schlcode)
```
...
```{r 2sls}
fit_2sls = feols(avgmath ~ csw0(pct_disadv, enrollment) | classize ~ iv_maimonides, 
      data = df,
      cluster = ~schlcode)
fit_2sls
```
...
```{r etable}
etable(fit_ols, fit_iv_rf, fit_2sls,
       se.below = TRUE,
       fitstat = ~ n + r2 + f,
       order = c("Intercept", "classize", "iv_maimonides"))
```
