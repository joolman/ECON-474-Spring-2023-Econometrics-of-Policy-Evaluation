...
cps = fread(paste0(path_data, 'DwD3 CPS degree earning 2010-2022.CSV'))
...
#   load in the data accounting for the structure of the data
#   - Option 1: use the `read_excel()` argument `skip`
#   - Option 2: use the `read_excel()` argument `range`
#
# hint: ?read_excel
?read_excel
cpi = read_excel(paste0(path_data, 'DwD3 CPI 2010-2022.xlsx'),
                 skip = 11)

read_excel(paste0(path_data, 'DwD3 CPI 2010-2022.xlsx'),
           range = 'A12:M25')
...
cps_clean = cps %>%
  mutate(earnweek = ifelse(earnweek == 9999.99, NA, earnweek)) %>%
  na.omit() %>%
  mutate(degree = case_when(educ == 125 ~ 'Ph.D.',
...
#   2. `summarise()` the variable `cpi_2019` to be the `mean()` of the variable `cpi`
cpi_2019 = cpi_long %>%
...
df_plot = inner_join(cps_clean, cpi_long) %>%
  bind_cols(cpi_2019) %>%
  mutate(r_earnweek = earnweek/cpi*cpi_2019) %>%
...
ggplot(df_plot, aes(x = date, y = real_earnweek,
  ...
  labs(x = '', y = 'Real Weekly Earnings ($2019)',
       ...
       subtitle = 'OMG this figure took so long to make...',
       caption = 'CPS weekly earnings and BLS CPI-U',
       color = 'Degree')
