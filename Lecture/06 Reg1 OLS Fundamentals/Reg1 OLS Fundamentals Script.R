...
fit_m = lm(log(hrly_wage) ~ yos + poly(exp, 2),
   data = cps)
...
cps = cps %>%
  mutate(degree = factor(degree),
         degree = fct_relevel(degree,
                              c("None", "High School", "Associate's", "Bachelor's",
...
lm(log(hrly_wage) ~ yos + exp + I(exp^2),
      data = cps) %>% summary
feols(log(hrly_wage) ~ yos + exp + I(exp^2),
      data = cps)

# using heteroskedastic standard errors is straightforward
feols(log(hrly_wage) ~ yos + exp + I(exp^2),
      data = cps, se = 'hetero')
# Notice the change to "Standard-errors" in the output

# You can run many regressions at once
feols(log(hrly_wage) ~ csw(yos + exp + I(exp^2), female*children, degree),
      data = cps, se = 'hetero')

# And with "multiple outcomes"
feols(c(renter, log(hrly_wage)) ~ csw(yos + exp + I(exp^2), female*children, degree),
      data = cps, se = 'hetero')
...
feols(log(hrly_wage) ~ yos + exp + I(exp^2) | statefip, 
      data = cps, se = 'hetero')
# note the intercept variable is gone since the data have been demeaned! (mean(y) == 0)

# Clustering standard errors requires inputting as a formula or string
feols(log(hrly_wage) ~ yos + exp + I(exp^2) | statefip,
      data = cps, cluster = ~ statefip) 
# the standard error shrank!
