...
# df = read_dta("C:/Users/JJPWade/Dropbox/Julian/Academia/UIUC/Semester 12 Spring 2023/ECON474sp23/Data/RDD2 amazon_data.dta")
df = read_dta("C:/Users/johnj/Dropbox/Julian/Academia/UIUC/Semester 12 Spring 2023/ECON474sp23/Data/RDD2 amazon_data.dta")

head(df) # show ntitle duplicates
```
...
df_plot_density = df %>%
  ...
  group_by(year, post1923) %>% # running variable and threshold variable
  summarise(n_books = n())
...
ggplot(df_plot_density2, aes(x = year, y = n_title, group = post1923)) +
  geom_point() + 
  geom_smooth(span = .8,
              degree = 2) +
...
ggplot(df_plot_fig1, aes(x = year, y = ntitle, group = post1923)) +
  geom_point() +
...
for(i in 1:length(formats)){
  fit_tab2_2_4[[i]] = feols(ntitleformat ~ post1923 + plr + prize_author + canon_title + canon_author + poly(year,3),
                         data = df_rdd, 
                         cluster = ~ year,
                         subset = df_rdd$format == formats[i])
}

etable(fit_tab2_1, fit_tab2_2_4,
       fitstat = ~ n + ar2,
       se.below = TRUE,
       extralines = list("_^Subset" = c("All Formats", formats)))
```
